const pieceImages = {
  white: {
    King: "pieces/wK.svg", Queen: "pieces/wQ.svg",
    Rook: "pieces/wR.svg", Bishop: "pieces/wB.svg",
    Knight: "pieces/wN.svg", Pawn: "pieces/wP.svg"
  },
  black: {
    King: "pieces/bK.svg", Queen: "pieces/bQ.svg",
    Rook: "pieces/bR.svg", Bishop: "pieces/bB.svg",
    Knight: "pieces/bN.svg", Pawn: "pieces/bP.svg"
  }
};
...
//This loops through all x64 grid boxes and gives/highlights the moves possible
    ...
    for (let row = 0; row < 8; row++) {
        for (let col = 0; col < 8; col++) {
            let allowed = isLegalMove(row, col, fromRow, fromCol, boardState, currentTurn)
            if (allowed) {
...
function renderHighlight(possibleMove) {
    ...
    possibleMove.forEach(move => {
        ...
        cells.forEach(cell => {
            if (Number(cell.dataset.row) === move.row && Number(cell.dataset.col) === move.col) {
...
function prevMoveHighlight(fromRow, fromCol, toRow, toCol) {
    ...
    cells.forEach(cell => {
        ...
        if (Number(cell.dataset.row) === fromRow && Number(cell.dataset.col) === fromCol) {
            cell.style.backgroundColor = "rgba(255, 255, 0, 0.25)"
        }
        if (Number(cell.dataset.row) === toRow && Number(cell.dataset.col) === toCol) {
...
function checkHighlight(kingRow, kingCol) {
    cells.forEach(cell => {
        ...
        const kingPosition = row === kingRow &&
        col === kingCol;
        if (kingPosition) {
