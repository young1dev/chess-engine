...
function getBotMove(boardState, color) {
        ...
        for (let fromCol = 0; fromCol < 8; fromCol++) {
            ...
            for (let toRow = 0; toRow < 8; toRow++) {
                for (let toCol = 0; toCol < 8; toCol++) {
                    ...
                    const legalMoves = isLegalMove(toRow, toCol, fromRow, fromCol, boardState, color)
                    if (legalMoves) {
    ...
    const botMove = allPossibleMoves[Math.floor (Math.random()*allPossibleMoves.length)]
    console.log(botMove)
    return botMove 
}
